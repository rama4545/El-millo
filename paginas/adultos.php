...
// Consultas por subcategorías
$remeras = $conn->query("SELECT * FROM productos WHERE categoria_id = 1 AND subcategoria_id = 1")->fetch_all(MYSQLI_ASSOC);
$camperas = $conn->query("SELECT * FROM productos WHERE categoria_id = 1 AND subcategoria_id = 2")->fetch_all(MYSQLI_ASSOC);
$shorts   = $conn->query("SELECT * FROM productos WHERE categoria_id = 1 AND subcategoria_id = 3")->fetch_all(MYSQLI_ASSOC);
?>
...
<head>
  <meta charset="UTF-8">
  <title>Adultos - El Millo</title>
  <link rel="icon" type="image/png" href="../img_index/favicon.png">
  <link rel="stylesheet" href="../css/adultos.css">
...
<main>
  <?php if ($esAdmin): ?>
    <div class="admin-botones">
      <a href="../php/agregar_producto.php?categoria=1" class="btn-agregar">+ Agregar producto</a>
      <a href="../php/editar_producto.php?categoria=1" class="btn-editar">Editar producto</a>
      <a href="../php/eliminar_producto.php?categoria=1" class="btn-eliminar-admin">Eliminar producto</a>
    </div>
  <?php endif; ?>
  ...
  <div class="seccion1">
    ...
    <section class="productos">
      <?php foreach ($remeras as $producto): ?>
        <div class="remeras-adultos">
...
<?php endif; ?>
  ...
  <div class="seccion2">
    ...
    <section class="productos">
      <?php foreach ($camperas as $producto): ?>
        <div class="remeras-adultos">
          <img src="../img productos/<?= htmlspecialchars($producto['imagen_principal']) ?>" alt="<?= htmlspecialchars($producto['nombre']) ?>">
          <h3><?= htmlspecialchars($producto['nombre']) ?></h3>
          <p>$<?= number_format($producto['precio'], 0, ',', '.') ?></p>
...
<?php endif; ?>
  ...
  <div class="seccion2">
    ...
    <section class="productos">
      <?php foreach ($shorts as $producto): ?>
        <div class="remeras-adultos">
          <img src="../img productos/<?= htmlspecialchars($producto['imagen_principal']) ?>" alt="<?= htmlspecialchars($producto['nombre']) ?>">
          <h3><?= htmlspecialchars($producto['nombre']) ?></h3>
          <p>$<?= number_format($producto['precio'], 0, ',', '.') ?></p>
