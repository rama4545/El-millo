...
$conn = (new Database())->getConnection();
$remerasNinos = $conn->query("SELECT * FROM productos WHERE categoria_id = 2 AND subcategoria_id = 4")->fetch_all(MYSQLI_ASSOC);
$camperasNinos = $conn->query("SELECT * FROM productos WHERE categoria_id = 2 AND subcategoria_id = 5")->fetch_all(MYSQLI_ASSOC);
$shortsNinos   = $conn->query("SELECT * FROM productos WHERE categoria_id = 2 AND subcategoria_id = 6")->fetch_all(MYSQLI_ASSOC);
?>
...
<?php if ($esAdmin): ?>
  <div class="admin-botones">
    <a href="../php/agregar_producto.php?categoria=2" class="btn-agregar">+ Agregar producto</a>
    <a href="../php/editar_producto.php?categoria=2" class="btn-editar">Editar producto</a>
    <a href="../php/eliminar_producto.php?categoria=2" class="btn-eliminar-admin">Eliminar producto</a>
  </div>
<?php endif; ?>
  ...
  <div class="seccion1">
    ...
    <section class="productos">
      <?php foreach ($remerasNinos as $producto): ?>
        <div class="niños">
...
<div class="seccion2">
  <div class="seccion-titulo">CAMPERAS</div>
  <section class="productos">
    <?php foreach ($camperasNinos as $producto): ?>
      <?php
        // Normalizamos el nombre para que no falle por mayúsculas ni tildes
        $nombreNormalizado = strtolower(trim($producto['nombre']));
        ...
        $claseImagen = '';
        if (strpos($nombreNormalizado, 'campera impermeable') !== false) {
          $claseImagen = 'img-campera-impermeable';
        }
      ?>
      <div class="niños">
        <img src="../img productos/<?= htmlspecialchars($producto['imagen_principal']) ?>"
...
<?php endif; ?>
  ...
  <div class="seccion2">
    <div class="seccion-titulo">SHORTS</div>
    <section class="productos">
      <?php foreach ($shortsNinos as $producto): ?>
        <div class="niños">
          <img src="../img productos/<?= htmlspecialchars($producto['imagen_principal']) ?>" alt="<?= htmlspecialchars($producto['nombre']) ?>">
          <h3><?= htmlspecialchars($producto['nombre']) ?></h3>
          <p>$<?= number_format($producto['precio'], 0, ',', '.') ?></p>
